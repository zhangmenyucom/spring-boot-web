...
public class SuoLiangXipanStrategy extends IStrategy {
    ...
    public int doCheck(List<HistoryData> historyData, String stockCode) {
        ...
        if (((yestoday.getClose() - theDaybefore.getClose()) / theDaybefore.getClose() >= 0.04 || (yestoday.getClose() - yestoday.getOpen()) / yestoday.getOpen() >= 0.03) && yestoday.getVolume().floatValue() / today.getVolume().floatValue() > 1.2 && ((today.getClose()-yestoday.getClose())/yestoday.getClose())>-0.06&& ((today.getClose()-yestoday.getClose())/yestoday.getClose())<0.02) {
            return 1;
        }
        return 0;
    }
